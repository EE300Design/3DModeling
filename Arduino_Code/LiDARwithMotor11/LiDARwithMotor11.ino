...
void loop() {
    ...
    delay(100);
    if(incomingByte.length()>0){     // echo to confirm command  recieved
      Serial.println(incomingByte); 
      }
      
    // if it's start command from GUI, begin scanning
    if (incomingByte.equals("start\n")) {
      start_scan();
      //Serial.flush();
    }
    // if it's stop command from GUI, stops scanning
